...
export const getSearchedOrders = (searchTerm = '', searchType, statusOptions) => async dispatch => {
    try {
...
export const cancelOrder = order => async dispatch => {
    try {
        const res = await axios.put(`http://localhost:8081/api/orders/update/${order.id}`,{
            userId: order.userId,
            status: "CANCELLED",
            paid: false
        })
